  		...
  		layui.use(['form','table','layer'],function(){
  			...
  			var table = layui.table,layer=layui.layer,$=layui.$;
  			 ...
  			 if(j.msgReply=="已回复"){
  			  	table.render({
  			  		text: {
						none: '数据已被删除' //默认：无数据。注：该属性为 layui 2.2.5 开始新增
					 },
					  initSort: {
	  				...
	  				cols:[[
	  						...
	  						{field:'askTime',title:'回复时间',align:'center'},
	  						{field:'userAsk',title:'回复内容',align:'center'},
	  						{fixed: 'right', title:'操作',align:'center', toolbar: '#demo', width:90}
	  					]]
  			})
  			}else{
  				  			  	table.render({
  			  		text: {
						none: '暂无留言数据' //默认：无数据。注：该属性为 layui 2.2.5 开始新增
					 },
	  				elem:'#tb',
	  				url:'../../../servlet/MessageServlet?op=searchUserMessage&msgId='+j.msgId,
	  				method:'post',
...
<body class="childrenBody">
	<form class="layui-form">
		<div class="replay_edit">
			<textarea class="layui-textarea" id="msgReply" name="userAsk" disabled="disabled"></textarea>
		</div>
	</form>
