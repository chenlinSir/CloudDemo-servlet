...
layui.config({
    ...
    //加载数据
    $.get("../../../servlet/MessageServlet?op=searchMessage",function(date){
    	data = eval('('+date+')');
    ...
    $("body").on("click",".reply_msg,.msgHtml .user_info h2,.msgHtml .msg_info>img",function(){
        ...
        var msgId = $(this).parents("tr").find(".user_info h3").text();
...
//                });
                ...
                //加载回复信息
                $.get("../../../servlet/MessageServlet?op=searchMessageById&msgId="+msgId,function(date){
                	data = eval('('+date+')');
