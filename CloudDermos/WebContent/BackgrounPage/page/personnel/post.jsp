  ...
  <head>
	...
	<script type="text/javascript">
		layui.use(['table','layer'],function(){
			...
			table.render({
				elem:'#tb',
				url:'servlet/staffinfoServlet?op=post',
				method:'post',
				page:true,
				limit:5,
				limits:[5,10,15],
				cols:[[
					{field:'post_id',title:'职位编号'},
					{field:'post_name',title:'职位名称',align:'center'},
					{field:'bh',title:'部门名称',templet:'<div>{{d.bh.b_name}}</div>',align:'center'},
					{field:'p_remark',title:'职位备注',align:'center'},
			...
			$("#addBtn").click(function(){
				$.post("servlet/staffinfoServlet?op=saddpost");
				layer.open({
					type:2,
					title:'添加职位',
					content:'BackgrounPage/page/personnel/addpost.jsp',
					area:['450px','400px'],
					shade:0.5,
					btn:'关闭',
					resize:false,//不允许拉伸
			...
			table.on('tool(show)',function(obj){
				...
				//将一个JavaScript值(对象或者数组)转换为一个 JSON字符串
					...
					layer.open({
						type:2,
						title:'修改职位',
						content:'BackgrounPage/page/personnel/updatepost.jsp',
						area:['450px','400px'],
						shade:0.5,
						btn:'关闭',
						resize:false,   //不允许拉伸
				...
				if(obj.event=='delete'){
					layer.confirm("是否删除该数据?",function(){
						$.post("servlet/staffinfoServlet",{op:'deletepost',id:data.post_id},function(res){
							if(res>0){
