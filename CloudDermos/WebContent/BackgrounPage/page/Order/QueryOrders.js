layui.config({
	...
	//加载页面数据
	var usersData = '';
	$.get("../../json/QueryOrders.json", function(data){
		usersData = data;
	...
	$(".search_btn").click(function(){
		...
		if($(".search_input").val() != ''){
            ...
            setTimeout(function(){
            	$.ajax({
					url : "servlet/OrderServlet?op=queryOrders",
					type : "get",
					dataType : "json",
					success : function(data){
						...
						for(var i=0;i<usersData.length;i++){
		            		...
		            		//接单员
		            		if(usersStr.sta_id.indexOf(selectStr) > -1){
			            		usersStr["sta_id"] = changeStr(usersStr.sta_id);
		            		}
		            		if(usersStr.Id.indexOf(selectStr)>-1 ||usersStr.userName.indexOf(selectStr)>-1 || usersStr.service_type.indexOf(selectStr)>-1 || usersStr.service_way.indexOf(selectStr)>-1 || usersStr.pay_type.indexOf(selectStr)>-1|| usersStr.order_state.indexOf(selectStr)>-1|| usersStr.sta_id.indexOf(selectStr)>-1){
		            			userArray.push(usersStr);
		            		}
		            		
		            	}
		            	usersData = userArray;
		            	QuerOrders(usersData);
					}
				})
